...
router.post("/settings", function (req, res, next) {
  ...
  req
    .checkBody("confirm_new_password", "Passwords do not match")
...
router.post("/forgot-password", function (req, res) {
  ...
  if (
    req.models.User.one({ email: req.param("email") }, function (err, user) {
      if (user) {
        var password = generateRandomPassword();
        var hash = generateHash(password);
        user.save({ password: hash }, function () {
          sendNewPassword(user, password);
          req.flash("success", "New password sent to your inbox");
          return res.redirect("/signin");
        });
      } else {
...
function findByUsername(username, fn) {
  orm.connect(settings.dsn, async function (err, db) {
